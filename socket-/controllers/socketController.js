...
const createPare = () => {
  function waiting(SOCKET, State, receve) {
    let createRoom = (socket, val) =>
      State.chain().createRoom(socket.id(), val).save().getRoom();
    try {
      SOCKET.joinRoom(
        createRoom(SOCKET, receve),
        'opponent-waiting',
        'waitNow'
      );
    } catch (error) {
  ...
  function startGame(SOCKET, State, receve) {
    try {
      let InRoomObj = State.chain()
        .getWatingPlayer()
        .roomIn(SOCKET.id(), receve)
        .save()
        .getRoom();
  ...
  function enterToRoom(SOCKET, roomVal, State) {
    ...
    SOCKET.joinRoom(
      roomVal.roomId,
      'opponent-find',
      State.getRoomObj(roomVal.roomId)[0]
    );
  }

  function enterRoom(SOCKET, roomVal, State) {
    State.isFullRoom(roomVal.roomId)
      ? SOCKET.emitToUser(SOCKET.id(), 'room-isFull')
      : enterToRoom(SOCKET, roomVal, State);
  }
  ...
  return (SOCKET, State) => {
    return {
      waitPare(receve) {
        ...
        isWating()
          ? waiting(SOCKET, State, receve)
          : startGame(SOCKET, State, receve);
        console.log('start');
      },
      disConnect() {
        ...
        if (roomData.roomId !== undefined) {
          roomData
            ? new GameStart(SOCKET.socket, SOCKET.io).emitRoom(
                'opponent-disconnect',
                'disconnect',
                roomData[0].roomId
              )
      ...
      enterRoom(roomVal) {
        console.log(State);
        State.isExistRoom(roomVal.roomId)
          ? enterRoom(SOCKET, roomVal, State)
          : SOCKET.emitToUser(SOCKET.id(), 'room-isnotExist');
      },
    };
  };
};
